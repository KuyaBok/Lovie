...
// PWA diagnostic helper - shows manifest, icons, and service worker status
async function showPwaDiagnostics() {
    try {
        const diag = document.createElement('div');
        diag.id = 'pwaDiagnostics';
        ...
        diag.innerHTML = '<strong>PWA diagnostics</strong><br>Checking...';
        document.body.appendChild(diag);

        // Check manifest
        let manifestStatus = 'manifest: not found';
        try {
            ...
            if (link && link.href) {
                ...
                if (res.ok) {
                    ...
                    manifestStatus = `manifest: ok (name: ${manifest.name || '—'})`;
                    // icons
                    const icons = manifest.icons || [];
                    manifestStatus += `<br>icons: ${icons.length}`;

                    // Try fetching each icon to ensure they are reachable
                    if (icons.length) {
                        for (const ic of icons) {
                            try {
                                const url = new URL(ic.src, link.href).href;
                                const r = await fetch(url, { method: 'HEAD', cache: 'no-store' });
                                manifestStatus += `<br> - ${url}: ${r.ok ? 'ok' : r.status}`;
                            } catch (e) {
                                manifestStatus += `<br> - ${ic.src}: error`;
                            }
                        }
                    }
                } else {
                    manifestStatus = `manifest: fetch failed (${res.status})`;
                }
            } else {
                manifestStatus = 'manifest: link missing';
            }
        } catch (e) {
            manifestStatus = 'manifest: error';
            console.error('Manifest check error', e);
        }

        // Check service worker registrations
        let swStatus = 'service worker: not supported';
        if ('serviceWorker' in navigator) {
            try {
                const regs = await navigator.serviceWorker.getRegistrations();
                swStatus = `service worker registrations: ${regs.length}`;
                if (regs.length > 0) {
                    swStatus += '<br> - active';
                }
            } catch (e) {
                swStatus = 'service worker: error';
            }
        }

        // Check secure context / localhost
        const secure = window.isSecureContext ? 'secure context: yes' : 'secure context: no (HTTPS or localhost required)';

        diag.innerHTML = `<strong>PWA diagnostics</strong><br>${manifestStatus}<br>${swStatus}<br>${secure}<br><br><small>Open DevTools Console for more details.</small>`;
        console.log('PWA diagnostics:', {manifestStatus, swStatus, secure});
    } catch (e) {
        console.error('Diagnostics failed', e);
    }
}

setTimeout(showPwaDiagnostics, 800);
